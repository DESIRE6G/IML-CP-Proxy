...
def sender_process(index: int, target_port: int, batch_size: int, rate_limit: int = None):
    print(f'sender_process started {index=} {target_port=} {batch_size=} {rate_limit=} part{index+1}')
    s = HighLevelSwitchConnection(index, f'part{index+1}', target_port, rate_limit=rate_limit)
    while True:
...
if __name__ == '__main__':
    for i in range(args.sender_num):
        ...
        process = multiprocessing.Process(target=sender_process, args=(i, int(args.target_port) + i, args.batch_size, rate_limit), daemon=True)
        process.start()
